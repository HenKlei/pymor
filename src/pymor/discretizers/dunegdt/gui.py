            ...
            if isinstance(U, tuple):
                ...
                if legend is not None and len(legend) == len(U):
                    ...
                    for ii in range(len(U)):
                        names.append(f'STATE_{ii}')
                data = OrderedDict()
                if interactive:
                    for ii in range(len(U)):
                        _, filename = mkstemp(suffix=f'_{ii}{suffix}')
                    ...
                    for ii in range(len(U)):
                        data[f'{filename}_{ii}{suffix}'] = U[ii]._list[0]
                ...
                if interactive:
                    ...
                    with open(pvd_filename, 'w') as pvd_file:
                        ...
                        part = 0
                        for f_name in data:
                            pvd_file.write(f'    <DataSet timestep="0" group="" part="{part}" file="{f_name}"/>\n')
                            part += 1
                        pvd_file.write('  </Collection>\n')
                ...
                if len(U) == 1:
                    ...
                    if interactive:
                        ...
                        assert len(filename) > 0
                        for ii in range(len(U)):
                            data[f'{filename}_{ii}{suffix}'] = U._list[ii]
                    ...
                    if interactive:
                        ...
                        with open(pvd_filename, 'w') as pvd_file:
                            ...
                            timestep = 0
                            for f_name in data:
                                pvd_file.write(f'    <DataSet timestep="{timestep}" group="" part="0" file="{f_name}"/>\n')
                                timestep += 1
                            pvd_file.write('  </Collection>\n')
                            pvd_file.write('</VTKFile>\n')
                        subprocess.call(['paraview', pvd_filename])
    ...
    class DuneGDTK3dVisualizer(ImmutableObject):
        ...
        def visualize(self, U, *args, **kwargs):
            ...
            if len(U) == 0:
                ...
                discrete_functions = [make_discrete_function(u) for u in U.impl._list]
                visualize_function(discrete_functions, self.grid)
